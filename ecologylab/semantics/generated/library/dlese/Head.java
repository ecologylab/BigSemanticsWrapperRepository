package ecologylab.semantics.generated.library.dlese;

import ecologylab.serialization.ElementState.xml_other_tags;
import ecologylab.semantics.metadata.scalar.MetadataString;
import ecologylab.semantics.generated.library.dlese.Collection;
import ecologylab.semantics.generated.library.dlese.AdditionalMetadata;
import ecologylab.semantics.metametadata.MetaMetadataCompositeField;
import ecologylab.serialization.Hint;
import java.util.List;
import java.util.Date;
import ecologylab.semantics.metadata.scalar.MetadataDate;
import ecologylab.semantics.metadata.Metadata;
import ecologylab.serialization.simpl_inherit;
import ecologylab.serialization.ElementState.xml_tag;

/**
*  Head.java
*  s.im.pl serialization
*
*  Generated by JavaTranslator on 08/10/11.
*  Copyright 2011 Interface Ecology Lab. 
*/

/** 
 */ 
@simpl_inherit
public class Head extends Metadata
{
	/** 
	 */ 
	@simpl_scalar	private ecologylab.semantics.metadata.scalar.MetadataString id;

	/** 
	 */ 
	@simpl_scalar	@xml_tag("fileLastModified")	private ecologylab.semantics.metadata.scalar.MetadataDate lastModified;

	/** 
	 */ 
	@simpl_scalar	@xml_tag("xmlFormat")	private ecologylab.semantics.metadata.scalar.MetadataString xmlFormat;

	/** 
	 */ 
	@simpl_composite @mm_name("collection")	private Collection collection;

	/** 
	 */ 
	@simpl_composite	@xml_tag("additionalMetadata") @mm_name("additional_metadata")	private AdditionalMetadata additionalMetadata;

	public Head()
	{ }

	public Head(MetaMetadataCompositeField mmd) {
		super(mmd);
	}


	public ecologylab.semantics.metadata.scalar.MetadataString	id()
	{
		ecologylab.semantics.metadata.scalar.MetadataString	result = this.id;
		if (result == null)
		{
			result = new ecologylab.semantics.metadata.scalar.MetadataString();
			this.id = result;
		}
		return result;
	}

	public java.lang.String getId()
	{
		return this.id == null ? null : id().getValue();
	}

	public ecologylab.semantics.metadata.scalar.MetadataString getIdMetadata()
	{
		return id;
	}

	public void setId(java.lang.String id)
	{
		if (id != null)
			this.id().setValue(id);
	}

	public void setIdMetadata(ecologylab.semantics.metadata.scalar.MetadataString id)
	{
		this.id = id;
	}

	public ecologylab.semantics.metadata.scalar.MetadataDate	lastModified()
	{
		ecologylab.semantics.metadata.scalar.MetadataDate	result = this.lastModified;
		if (result == null)
		{
			result = new ecologylab.semantics.metadata.scalar.MetadataDate();
			this.lastModified = result;
		}
		return result;
	}

	public java.util.Date getLastModified()
	{
		return this.lastModified == null ? null : lastModified().getValue();
	}

	public ecologylab.semantics.metadata.scalar.MetadataDate getLastModifiedMetadata()
	{
		return lastModified;
	}

	public void setLastModified(java.util.Date lastModified)
	{
		if (lastModified != null)
			this.lastModified().setValue(lastModified);
	}

	public void setLastModifiedMetadata(ecologylab.semantics.metadata.scalar.MetadataDate lastModified)
	{
		this.lastModified = lastModified;
	}

	public ecologylab.semantics.metadata.scalar.MetadataString	xmlFormat()
	{
		ecologylab.semantics.metadata.scalar.MetadataString	result = this.xmlFormat;
		if (result == null)
		{
			result = new ecologylab.semantics.metadata.scalar.MetadataString();
			this.xmlFormat = result;
		}
		return result;
	}

	public java.lang.String getXmlFormat()
	{
		return this.xmlFormat == null ? null : xmlFormat().getValue();
	}

	public ecologylab.semantics.metadata.scalar.MetadataString getXmlFormatMetadata()
	{
		return xmlFormat;
	}

	public void setXmlFormat(java.lang.String xmlFormat)
	{
		if (xmlFormat != null)
			this.xmlFormat().setValue(xmlFormat);
	}

	public void setXmlFormatMetadata(ecologylab.semantics.metadata.scalar.MetadataString xmlFormat)
	{
		this.xmlFormat = xmlFormat;
	}

	public Collection getCollection()
	{
		return collection;
	}

	public void setCollection(Collection collection)
	{
		this.collection = collection;
	}

	public AdditionalMetadata getAdditionalMetadata()
	{
		return additionalMetadata;
	}

	public void setAdditionalMetadata(AdditionalMetadata additionalMetadata)
	{
		this.additionalMetadata = additionalMetadata;
	}
}
